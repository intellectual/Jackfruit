...
def chooseCascade(choice='frontalFace'):
    ...
    # HAAR cascades 
    if choice is 'frontalFace':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_frontalface_alt2.xml')
    elif choice is 'profileFace':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_profileface.xml')
    elif choice is 'leftEye':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_leftEye.xml')
    elif choice is 'rightEye':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_rightEye.xml')
    elif choice is 'rightEye2':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_righteye_2splits.xml')
    elif choice is 'frontalEyes':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_frontalEyes.xml')
    elif choice is 'eyeGlasses':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_eye_tree_eyeglasses.xml')
    elif choice is 'fullBody':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_fullbody.xml')
    elif choice is 'upperBody':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_upperbody.xml')
    elif choice is 'lowerBody':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_lowerbody.xml')
    elif choice is 'head':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_head_16x16.xml')
    elif choice is 'nose':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_nose_25x15.xml')
    elif choice is 'mouth':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/haarcascade_mcs_mouth.xml') # doesn't work properly
    
    # LBP cascades    
    elif choice is 'frontalFace_LBP':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/lbpcascade_frontalface.xml')
    elif choice is 'mascfrontalFace_LBP':
        _cascade = cv2.CascadeClassifier('./cascadeFiles/lbpcascade_mascFrontalFace_20x20_Nov-22-2013.xml')
    else:
        _cascade = None
    return _cascade
...
def saveCamImage():
    ...
    while(keyPressed != 27):
        ...
        if keyPressed == ord('s'):
            cv2.imwrite('/Users/samkhan/Documents/faceImages/frontalFaces/' +
                    'faceImage' + str(saveCount) + '.jpg', camImage)
            saveCount += 1
...
def drawMatches(img1,kp1,img2,kp2,matches):
    ...
    # outShape1 = (femaleImg.shape[0],femaleImg.shape[1],3)
    # outShape2 = (maleImg.shape[0],maleImg.shape[1],3)
    
    # out1 = np.zeros(femaleImg.shape)
    # out2 = np.zeros(maleImg.shape)
    
    # femaleImg = cv2.drawKeypoints(femaleImg,femaleKP)
    # maleImg = cv2.drawKeypoints(maleImg,maleKP)
    
     
    # for idx, kp in enumerate(kp1):
    #     kpX, kpY = femaleKP[idx].pt
    #     radius = int(femaleKP[idx].size)
    #     cv2.circle(femaleImg,(int(kpX),int(kpY)),1,(0,0,255),-1)
    #       
    # for idx, kp in enumerate(kp2):
    #     kpX, kpY = maleKP[idx].pt
    #     radius = int(maleKP[idx].size)
    #     cv2.circle(maleImg,(int(kpX),int(kpY)),1,(0,0,255),-1)
    # 
    # cv2.imwrite('femaleImg.jpg',femaleImg)
    # cv2.imwrite('maleImg.jpg',maleImg)